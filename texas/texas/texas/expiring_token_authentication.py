...
# so that users will have to re-authenticate after 12 hours of inactivity.
...
class ExpiringTokenAuthentication(TokenAuthentication):
    def authenticate_credentials(self, key):
        ...
        if (token.created < (datetime.now(datetime.timezone.utc) - datetime.timedelta(hours=12))):
