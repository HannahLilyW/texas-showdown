...
const props = defineProps(['background_color', 'shirt_color', 'skin_color', 'hat_color', 'small'])

const canvasSize = props.small ? 50 : 100;
...
function drawShirt(ctx: CanvasRenderingContext2D) {
    if (props.shirt_color != 'blank') {
        ...
        ctx.beginPath();
        ctx.arc(canvasSize / 2, canvasSize * 1.2, canvasSize / 2, 0, 2 * Math.PI);
        ctx.fill();
    }
}
...
function drawSkin(ctx: CanvasRenderingContext2D) {
    if (props.skin_color != 'blank') {
        ...
        ctx.beginPath();
        ctx.arc(canvasSize / 2, canvasSize / 2, canvasSize / 4, 0, 2 * Math.PI);
        ctx.fill();
    }
}
...
function drawHat(ctx: CanvasRenderingContext2D) {
    ctx.save();
    ctx.scale((canvasSize * 0.7)/32,(canvasSize * 0.7)/32);
    ctx.translate(15,-6);
    ctx.rotate(0.4);
...
<template>
<canvas :height="canvasSize" :width="canvasSize" id="profilePicCanvas" ref="profilePicCanvas"></canvas>
</template>
